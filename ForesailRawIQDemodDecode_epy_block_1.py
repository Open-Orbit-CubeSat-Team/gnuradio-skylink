...
class blk(gr.sync_block):
    ...
    VC_NAMES = {
        0: "VC0 Commanding",
        1: "VC1 Mass data transfer",
        2: "VC2 Service channel",
        3: "VC3 HAM repeater",
    }
    ...
    def _emit(self, de255: bytes, golay_raw: int, data12: int, n_errors: int, asm_abs: int):
        ...
        # Fix: VC is 2 bits, not 3
        vc          = flags_vc & 0x03

        # Keep original indexing/behavior for the rest so output stays familiar
        ...
        clean_auth, clean_payload = (
            (after_ext[-8:], after_ext[:-8])
            if has_auth and len(after_ext) >= 8 else (b"", after_ext)
        )

        vc_label = self.VC_NAMES.get(vc, f"VC{vc}")
        ...
        if clean_ext:
            print(f"  Ext header  : {clean_ext.hex()}")
        if clean_payload:
        ...
        if clean_auth:
            print(f"  Auth (8B)   : {clean_auth.hex()}")
    ...
    def work(self, input_items, output_items):
        ...
        while True:
            ...
            if len(golay_bits) < self.GOLAY_LEN or len(cw_bits) < self._cw_bits:
                print(f"[reject] short buffer after ASM @ bit {self._abs0 + asm_rel}")
                break

            def reject(reason):
                nonlocal rel
                print(f"[reject] ASM @ bit {self._abs0 + asm_rel}: {reason}")
                rel = asm_rel + 1
            ...
            try:
                ...
                reject("Golay uncorrectable")
                continue

            data_len = data12 - self.RS_BYTES
            if not (1 <= data_len <= self.DAT_BYTES):
                reject(f"bad data_len={data_len}")
                continue

            raw255 = self._pack_msb(cw_bits)
            de255  = self._dewhiten(raw255)

            if de255[:7] != self.MAGIC:
                reject(f"bad magic {de255[:7].hex()}")
                continue

            if (11 + de255[8]) > self.DAT_BYTES:
                reject(f"ext_len too large: ext_len={de255[8]}")
                continue
